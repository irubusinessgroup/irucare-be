...
export function renderPurchasesReport(
  data: PurchasesReportData,
): Promise<Buffer> {
  const { company, invoices } = data;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(18)
        .fillColor(BLACK)
        .text("Purchases REPORT", marginX, 20 * MM, {
          align: "center",
          width: tableW,
        });
      ...
      const fixedPt = (8 + 28 + 38 + 30 + 28 + 30 + 20) * MM;
      const autoW = tableW - fixedPt;

      const cols: ColDef[] = [
        { header: "#", width: 8 * MM, align: "center" },
        { header: "Supplier TIN", width: 28 * MM, align: "center" },
        { header: "Supplier Name", width: 38 * MM, align: "left" },
        { header: "Invoice Number", width: 30 * MM, align: "center" },
        { header: "Request Date", width: 28 * MM, align: "center" },
        { header: "Total Amount", width: 30 * MM, align: "right" },
        { header: "Item Name", width: autoW, align: "left" },
        { header: "Qty", width: 20 * MM, align: "center" },
      ];
