...
export const generateQrCodeData = (sell: any) => {
  ...
  const isRefund = sell.type === "REFUND" || !!sell.parentSellId;
  const label = isRefund ? "NR" : "NS";
  const formattedReceiptNumber = sell.totRcptNo
    ? `${sell.rcptNo}/${sell.totRcptNo} ${label}`
    : `${sell.rcptNo} ${label}`;

  return `${invoiceDate}#${time}#${sell.sdcId}#${formattedReceiptNumber}#${sell.intrlData}#${sell.rcptSign}`;
};
